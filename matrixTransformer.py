...
def handleRows(PSSM, SWITCH, COUNT):
    ...
    for i in range(PSSM_shape[0]):
        ...
        str_vec_positive = [int(float(i)) for i in str_vec[1:21]]
        ...
        #print str_vec_positive
        if COUNT==20:
            matrix_final[0]=sum(str_vec_positive, matrix_final[0])
        elif COUNT==400:
            matrix_final[Amino_vec.index(str_vec[0])] = sum(str_vec_positive, matrix_final[Amino_vec.index(str_vec[0])])
